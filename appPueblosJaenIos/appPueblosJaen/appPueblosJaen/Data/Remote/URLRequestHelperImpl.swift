...
final class URLRequestHelperImpl: URLRequestHelperProtocol {
    ...
    func listNegocios(puebloId: Int) -> URLRequest? {
        var components = URLComponents(string: "\(baseURL)\(endpoints.negocios)")
        components?.queryItems = [
            URLQueryItem(name: "pueblo_id", value: "eq.\(puebloId)"),
            URLQueryItem(name: "activo", value: "is.true"),
            URLQueryItem(name: "order", value: "orden_destacado.asc, nombre.asc")
        ]
        guard let url = components?.url else {
            print("Error al crear la url de negocios: \(String(describing: components))")
            return nil
        }
